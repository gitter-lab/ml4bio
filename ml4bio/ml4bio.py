...
# https://stackoverflow.com/questioimport sys
import warnings
from pandas import errors
from sklearn import tree, neighbors, linear_model, neural_network, svm, naive_bayes, exceptions
# Ignore specific DeprecationWarning as in
...
class App(QMainWindow):
    ...
    def load(self, labeled):
        ...
        :type labeled: bool
        """
        path = QFileDialog.getOpenFileName(self.dataPage, directory='../data/')[0]
        if path != '':
            # add labeled data.
            if labeled:
                try:
    ...
    def initUI(self):
        ...
        Sets up the GUI.
        """
        ### Type cast w and h to ints to get rid of float problem
        # global geometry
        sizeObject = QDesktopWidget().screenGeometry(-1)
        w = int(sizeObject.width()*0.5)
        h = int(sizeObject.height()*0.5)
        self.resize(w, h)
        ...
        self.font = QFont()
        # font for table and summary
        self.font.setPointSize(14)
        self.font.setPixelSize(self.font.pointSize())
        #self.font = QFontMetrics(self.font)

        ########## 1st page: data loading and splitting ##########
        self.dataPage = QWidget()
        self.dataPage.setStyleSheet("QLabel {font: 19px} QPushButton {font: 16px} QRadioButton::indicator {height: 40px; width: 40px}")
        #self.dataPage.font.setPointSize(10)
        #self.dataPage.font.setPixelSize(self.font.pointSize())
        ...
        #self.labeledFilePushButton.setStyleSheet("QLabel {font: 19px}")
        ...
        #self.unlabeledFilePushButton.setStyleSheet("QLabel {font: 19px}")
        ...
        self.splitFrame.setDisabled(True)
        self.splitFrame.setStyleSheet("QCheckBox {font: 19px; image: url(ml4bio/icons/check.png)} QLabel {font: 19px} ")
        self.splitSpinBox = self.spin_box(20, 10, 50, 10, self.splitFrame)
        self.splitSpinBox.setMaximumWidth(60)
        self.splitSpinBox.setStyleSheet("QSpinBox {font: 16px}")
        ...
        self.trainTestLayout = QGridLayout()
        self.trainTestLayout.addWidget(self.splitSpinBox, 0 ,0)
        self.trainTestLayout.addWidget(self.splitLabel, 0, 1)
        self.trainTestLayout.addWidget(self.splitCheckBox, 0, 2)
        ...
        self.validationFrame.setStyleSheet("QRadioButton {font: 19px} QRadioButton::indicator {height: 40px; width: 40px} QLabel {font: 19px} QSpinBox {font: 16px}")
        ...
        self.holdoutSpacer = QSpacerItem(12, 12, QSizePolicy.Minimum, QSizePolicy.Minimum)
        self.cvSpacer = QSpacerItem(12, 12, QSizePolicy.Minimum, QSizePolicy.Minimum)
        self.looSpacer = QSpacerItem(12, 12, QSizePolicy.Minimum, QSizePolicy.Minimum)
        self.validationSpacer = QSpacerItem(12, 12, QSizePolicy.Minimum, QSizePolicy.Minimum)
        self.holdoutSpinBox = self.spin_box(20, 10, 50, 10, self.validationFrame)
        self.holdoutSpinBox.setMaximumWidth(60)
        self.cvSpinBox = self.spin_box(5, 2, 10, 1, self.validationFrame)
        self.cvSpinBox.setMaximumWidth(60)
        self.cvSpinBox.setDisabled(True)
        ...
        self.validationCheckBox.setChecked(True)
        self.validationCheckBox.setStyleSheet("QCheckBox {font: 19px}")
        ...
        # Layout for Holdout Validation 
        self.holdoutLayout = QGridLayout()
        self.holdoutLayout.addWidget(self.holdoutRadioButton, 0, 0)
        self.holdoutLayout.addItem(self.holdoutSpacer, 0, 1)
        self.holdoutLayout.addWidget(self.holdoutSpinBox, 0, 2)
        self.holdoutLayout.addWidget(self.holdoutLabel, 0, 3)
        
        # Layout for Cross Validation Options 
        self.cvLayout = QGridLayout()
        self.cvLayout.addWidget(self.cvRadioButton, 0, 0)
        self.cvLayout.addItem(self.cvSpacer, 0, 1)
        self.cvLayout.addWidget(self.cvSpinBox, 0, 2)
        self.cvLayout.addWidget(self.cvLabel, 0, 3)
        
        # Layout for Leave-One-Out Validation Option
        self.looLayout = QHBoxLayout()
        self.looLayout.addWidget(self.looRadioButton)
        #self.looLayout.addItem(self.looSpacer)
        
        # Layout for Stratified Sampling Option
        ...
        # Combining all the layouts into one 
        ...
        self.validationLayout.addItem(self.validationSpacer)
        ...
        ## Fixing the fonts for models section --- need to make them all smaller
        self.modelPage.setStyleSheet("QLabel {font: 16px} QComboBox {font: 16px} QSpinBox {font: 16px} QLineEdit {font: 16px} QCheckBox::indicator {width: 40px; height: 40px} QPushButton {font: 16px} QDoubleSpinBox {font: 16px}")
        ## Make the checkboxs bigger
        ...
        ## The comments in this section are for relics that are needed for the QHBoxLayout --- if uncommented  it will create overlapping text
        self.lrStopLabel = QLabel('Stopping Criteria:', self.lrPage)
        #self.lrTolLabel = QLabel('tol:', self.lrPage)
        #self.lrTolLabel.setMinimumWidth(50)
        self.lrTolLineEdit = QLineEdit('1e-3', self.lrPage)
        #self.lrMaxIterLabel = QLabel('max_iter:', self.lrPage)
        #self.lrMaxIterLabel.setStyleSheet("QLabel {font: 15pt}")
        #self.lrMaxIterLabel.setMinimumWidth(50)
        #self.lrMaxIterLabel.setMaximumWidth(25)
        ...
        ## Turned the LR into a form for easier reading
        self.lrStopLayout = QFormLayout()
        self.lrStopLayout.addRow('tol:', self.lrTolLineEdit)
        self.lrStopLayout.addRow('max_iter:', self.lrMaxIterLineEdit)
        ...
        ## Turned the NN ending into a form for easier reading 
        self.nnStopLayout = QFormLayout()
        self.nnStopLayout.addRow('tol:', self.nnTolLineEdit)
        self.nnStopLayout.addRow('max_iter:', self.nnMaxIterLineEdit)
        ...
        ## Turned the SVM ending into a form for easier reading 
        self.svmStopLayout = QFormLayout()
        self.svmLayout.addRow('tol:', self.svmTolLineEdit)
        self.svmLayout.addRow('max_iter:', self.svmMaxIterLineEdit)
        ...
        self.testPage.setStyleSheet("QLabel {font: 16px} QComboBox {font: 16px} QSpinBox {font: 16px} QRadioButton {font: 16px} QPushButton {font: 16px} QDoubleSpinBox {font: 16px}")
        ...
        for i in range(len(headerList)):
            self.models_table.setColumnWidth(i, fm.width(headerList[i])+60)
        ...
        self.visFrame.setStyleSheet("QRadioButton {font: 16px}")
        ...
        self.canvas.setMaximumHeight(int(w*0.28))
...
if __name__ == '__main__':
    main()
ns/879173/how-to-ignore-deprecation-warnings-in-python
with warnings.catch_warnings():
    warnings.filterwarnings("ignore",category=DeprecationWarning)
    from sklearn import ensemble
from matplotlib.backends.backend_qt5agg import FigureCanvas
